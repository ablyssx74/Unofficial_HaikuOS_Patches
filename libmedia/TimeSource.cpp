...
Switch to math to (double) and remove rigid FLT_MANT_DIG check, 
solves the legacy precision bottleneck that was originally designed when CPUs 
and audio hardware were much slower and less stable.

Even if the hardware clock drifts slightly, your division-based calculation maintains 
accuracy over a much longer period than the original linear logic.
	...
	bigtime_t with_latency)
{
	...
	// 1. Get the last known timing synchronization point
	if (GetTime(&last_perf_time, &last_real_time, &last_drift) != B_OK)
		debugger("BTimeSource::RealTimeFor: GetTime failed");

	bigtime_t perf_time_difference = performance_time - last_perf_time;

	/* Uncomment if you really care
	
	static int32 print_counter = 0;
	if (print_counter++ % 500 == 0) {
		fprintf(stderr, "LIBMEDIA_DEBUG: diff=%" B_PRId64 " drift=%.6f\n", 
			perf_time_difference, last_drift);
	}
	*/
	
	// 3. Fallback check: If drift is zero, we avoid division by zero crash
	if (last_drift == 0.0f) {
		fprintf(stderr, "LIBMEDIA_DEBUG: Warning - Drift is zero!\n");
		return last_real_time - with_latency + perf_time_difference;
	}

	// 4. Calculate real time using double precision to bypass 
	// the original 24-bit float mantissa limitation.
	// The debugger() call is removed to prevent the crash.
	return last_real_time - with_latency 
		+ (bigtime_t)((double)perf_time_difference / (double)last_drift);
}
	...
	bigtime_t with_latency)
{
	...
	return last_real_time - with_latency + (bigtime_t)(perf_time_difference / last_drift);
}
*/
